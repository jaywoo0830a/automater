...
자동으로 분석해서 selectors/naver/editor.json (또는 login.json)에 저장합니다.
...
------
    ...
    python scripts/capture_selectors.py --output selectors/naver/login.json
    python scripts/capture_selectors.py --output selectors/naver/editor.json --merge
"""
...
def save_json(path: Path, data: dict) -> None:
    out: dict = {"_comment":[
        "Naver selectors — generated by scripts/capture_selectors.py",
        "locators are ordered most-stable → least-stable.",
    ]}
    out.update(data)
...
def run_capture(page: Page, output: Path, merge: bool) -> None:
    captured: dict = load_json(output) if merge else {}
...
def main() -> None:
    ...
    parser.add_argument("--output", default="selectors/naver/editor.json")
    parser.add_argument("--merge",  action="store_true",
                        help="기존 JSON에 추가 (기존 키 유지)")
