...
# ---------------------------------------------------------------------------
# Debug: align + image → text 입력 실패 재현
# ---------------------------------------------------------------------------

@pytest.mark.slow
@pytest.mark.parametrize("align", ["center", "left", "right"])
def test_debug_align_image_then_text(editor: SmartEditorOne, account: AccountOption, align: str):
    """정렬 + 이미지 업로드 후 텍스트 입력 — 프로덕션 실패 시나리오 재현."""
    image = _asset("images", 0)
    if not image:
    ...
    spec = PostingSpec(
        account=account,
        title=f"debug align+image ({align})",
        body=(Section(blocks=(
            ImageBlock(path=image),
            ParagraphBlock(prompt="이미지 뒤 본문 입력 테스트"),
        )),),
        align=align,
    )
    _run_spec(spec, editor)
...
@pytest.mark.slow
def test_debug_align_center_multi_image_text(editor: SmartEditorOne, account: AccountOption):
    """가운데 정렬 + 이미지 2장 + 텍스트 — 반복 실패 재현."""
    image = _asset("images", 0)
    if not image:
        pytest.skip("No image in assets/images/")
    ...
    spec = PostingSpec(
        account=account,
        title="debug center + multi image",
        body=(Section(blocks=(
            ImageBlock(path=image),
            ParagraphBlock(prompt="첫 번째 이미지 뒤 본문"),
            ImageBlock(path=image),
            ParagraphBlock(prompt="두 번째 이미지 뒤 본문"),
        )),),
        align="center",
    )
    _run_spec(spec, editor)
...
@pytest.mark.slow
def test_debug_align_center_manual_steps(editor: SmartEditorOne, page: Page):
    """수동 단계별 실행 — 어느 시점에서 실패하는지 확인."""
    from automator.browser_actions import click_if_visible
    import time
    ...
    editor.open()
    editor.set_align("center")
    print("\n  [1] set_align done")

    editor.write_title("debug manual steps")
    print("  [2] write_title done")

    # Step: upload image
    editor.upload_file(image)
    print("  [3] upload_file done")

    # Step: move_cursor (this is what runner does before the next block)
    editor.move_cursor("end")
    print("  [4] move_cursor done")

    # Step: check paragraph is clickable
    frame = editor._frame()
    sel   = editor._sel()

    para = sel.locator(frame, "editor_paragraph_container").last
    print(f"  [5] paragraph count: {sel.locator(frame, 'editor_paragraph_container').count()}")

    # Check blurred overlay
    blurred = frame.locator("div.se-selection.se-is-blurred")
    blurred_count = blurred.count()
    blurred_visible = blurred_count > 0 and blurred.first.is_visible()
    print(f"  [6] se-is-blurred count={blurred_count} visible={blurred_visible}")

    # Try typing
    try:
        para.click(timeout=5_000)
        print("  [7] paragraph click: OK")
    except Exception as e:
        print(f"  [7] paragraph click: FAILED — {e}")
        try:
            para.click(force=True, timeout=5_000)
            print("  [7b] force click: OK")
        except Exception as e2:
            print(f"  [7b] force click: FAILED — {e2}")

    page.keyboard.type("테스트 텍스트 입력")
    time.sleep(0.5)

    # Verify text was inserted
    text_content = para.text_content() or ""
    has_text = "테스트" in text_content
    print(f"  [8] text in paragraph: {has_text} (content: {text_content[:80]})")


@pytest.mark.slow
def test_debug_no_align_image_then_text(editor: SmartEditorOne, account: AccountOption):
    """정렬 없이 이미지 + 텍스트 — 정렬이 원인인지 확인용 대조군."""
    image = _asset("images", 0)
    if not image:
        pytest.skip("No image in assets/images/")
    ...
    spec = PostingSpec(
        account=account,
        title="debug no-align + image",
        body=(Section(blocks=(
            ImageBlock(path=image),
            ParagraphBlock(prompt="정렬 없이 이미지 뒤 본문"),
        )),),
    )
    _run_spec(spec, editor)
